...
namespace Graphic
{
	Font::Font(std::string _fontName) :
		...
		fontSampler(Graphic::SamplerState::EDGE_TREATMENT::WRAP, Graphic::SamplerState::SAMPLE::LINEAR,
				Graphic::SamplerState::SAMPLE::LINEAR, Graphic::SamplerState::SAMPLE::LINEAR)
	{
		m_effect.BindTexture( "u_characterTex", 7,fontSampler);
...
//			CharVertex[i].position += CharVertex[i].position - _screenPos; 
	...
	void TextRender::RenewBuffer()
	{
		...
		for(int i = 0; i<m_text.length(); i++)
		{
			CharacterVertex* CV = new CharacterVertex;
			CV->position = 0.1f*i;//currentPos;
			CV->scale = 1.f; 
